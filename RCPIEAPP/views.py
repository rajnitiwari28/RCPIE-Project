...
from .report_generator import RCReportGenerator
import os
...
def generate_report(request):
    """Generate and download branch-wise Excel report"""
    try:
        # Check if user has permission (RC Convener or Admin)
        if not (request.user.groups.filter(name='RC_Convener').exists() or request.user.is_staff):
            messages.error(request, "You don't have permission to generate reports.")
            return redirect('rc_convener_home')
        
        # Generate the report
        generator = RCReportGenerator()
        ...
        response['Content-Disposition'] = f'attachment; filename="RC_Submissions_Report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx"'
